...
def check_keydown_events(event, ai_settings, screen, ship, bullets, 
    ...
    if event.key == pygame.K_RIGHT:
        ...
        sign = ">" if stats.game_pause else "||"
        pause_button.prep_msg(sign)
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            check_keydown_events(event, ai_settings, screen,
                                 ship, bullets, stats, pause_button)
...
def check_play_button(ai_settings, screen, stats, sb, play_button, ship,
    ...
    # Метод collidepoint(x, y) определяет находится ли клик мыши
    # в области кнопки Play
...
def check_bullet_alien_collisions(ai_settings, screen, stats, sb, ship,
                                  aliens, bullets):
    '''Обработка коллизиц пуль с пришельцами.'''
...
def get_number_aliens_x(ai_settings, alien_width):
    '''Вычисляет колтчество пришельцев в ряду.'''
...
def update_aliens(ai_settings, screen, stats, sb, ship,
    ...
    '''Проверяет, достиг ли флот края экрана,
        после обновляет позиции всех пришельцев во флоте.'''
    check_fleet_edges(ai_settings, ship, aliens)
    aliens.update()
    # Проверка коллизий "пршелец-корабль".
